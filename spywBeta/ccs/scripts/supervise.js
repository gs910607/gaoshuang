		...
		try{
			...
			if(userdata && userdata.usergroupid.toString().length < 3){
				...
				$(".videosite").click(function(){
					window.location.href = "../videoMonitoring/videoPointInfo.html?type=2" 
				});
			}else{
	...
	var _zTreeOnExpand = function(event, treeId, treeNode) {
		if(treeNode.code.length < 9){
			$.post("/spywBeta/area/arealistByChildrens.do", {parentId:treeNode.code}, function(data) {
				if(data.status == 1){
					$.post("/spywBeta/monitoring/selectInfo.do",{videoinfoType:2,videoinfoCode:treeNode.code},function(dataInfo){
			...
			$.post("/spywBeta/monitoring/selectInfo.do", {videoinfoType:2,videoinfoCode:treeNode.code}, function(data) {
				if(data.status == 1){
					if(data.list.length > 0){
						var groupNodeList = [];
						for(i =0 ; data.list.length > i ; i++){
