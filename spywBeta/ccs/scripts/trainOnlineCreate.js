	...
	methods: {
		...
		createSureBtn: function() { //确认创建
			...
			if(this.checkedNames.length <= 0) {
				...
				var accessCode=$("#conferCode").val();
				var isRecording=$("#recorded").val();
				var data={name:name,confstartTime:beginTime,duration:duration,accessCode:accessCode,isRecording:isRecording,confersite:JSON.stringify(checkedNames),type:5};
				$.ajax({
					url : "/spywBeta/videoTraining/videoConferInsert.do", //请求的url地址 confersite:JSON.stringify(checkedNames)
					...
					success : function(responseStr) {
						...
						if (responseStr.status == 1) {
							...
							setTimeout(function() {
								location.href = "trainOnlineList.html";
							}, 1000)
						} else {
							alert(responseStr.msg);
