...
function init() {
	...
	$.ajax({
		type: "get",
		data: param,
		url: config.appserver_url + "/myClinic/queryMyHospitalVo.json",
		dataType: "json",
		async: false,
...
//			tokenLose(data.status);
			...
			$("#dialMain-first>p").text("您确认要拨打" + data.data.hospitalVo.contactsPhone + "吗？")
			//轮播图首图片更换
	...
	function initDoctor() {
		...
		$.ajax({
			type: "get",
			data: parmas,
			url: config.appserver_url + "/myClinic/getHospitalDoctor.json",
			dataType: "json",
			async: true,
			success: function(data) {
				//遍历诊所医护
