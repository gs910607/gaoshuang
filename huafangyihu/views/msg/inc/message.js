...
function initParams() {
	...
	serviceOrder = JSON.parse(sessionStorage.getItem("serviceOrder"));
}
...
function initChat() {
	...
	RongIMClient.connect(user.rongCloudToken, {
		onSuccess: function(userId) {
			...
			// 咨询内容
			if(!Validator.validateNull(chatInfo)) {
				sendMsg(chatInfo.content);
			}

			if(!Validator.validateNull(chatInfo.picList)) {
				var imgMaxWidth = $(window).width() - 50;
				$(".bubble").css("max-width", imgMaxWidth)

				for(var i = 0; i < chatInfo.picListSize; i++) {
					var picJson = JSON.parse(chatInfo.picList[i]);
					sendMsgImg(picJson.filePath, picJson.fileBase64);
				}
...
function sendMsgImg(filePathUrl, base64Str) {
	...
	var targetId = chatInfo.targetId; // 目标 Id
	RongIMClient.getInstance().sendMessage(conversationtype, targetId, msg, {
		onSuccess: function(message) {
			...
			var consultationRequest = {
				...
				type: 1,
				serviceOrderNo: chatInfo.serviceOrderNo,
				patientId: user.userId,
				doctorNurseId: chatInfo.doctorNurseId,
...
function sendMsg(content) {
	...
	if(!Validator.validateNull(content)) {
		...
		RongIMClient.getInstance().sendMessage(conversationtype, chatInfo.targetId, msg, {
			// 发送消息成功
			onSuccess: function(message) {
				//message 为发送的消息对象并且包含服务器返回的消息唯一Id和发送消息时间戳
				console.log("Send successfully");
...
function initPages() {
	// 初始化医护简介
	var params = {
		doctorId: chatInfo.doctorNurseId,
		//      doctorId: "zk1",
		token: user.token
	};
