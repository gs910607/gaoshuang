
var cityCode;
var token;
var id1;
var id2;
var hospitalId;
//获取url的hospitalId的值
 hospitalId = GetQueryStr("hospitalId");
 console.log(hospitalId)
getItems();
id1;
init3('Drug_delivery',0);

// 左侧初始化渲染
function init3(id1,id2) {
	var params = {
		cityCode: cityCode,
		id: id1,
		token: token
	};
	console.log(params)
	// $("#"+id1).addClass("footer-hover");
	$("li[data-id2="+ id2 +"]").find("a").addClass("picactive");
    this.id1=id1;
	$.ajax({
		type: 'GET',
		url: config.appserver_url + '/serviceItem/queryServiceCategoryType.json',
		data: params,
		dataType: 'json',
		success: function(response) {
			console.log(response.data)
			// 二级菜单（左面）
			var arr = response.data;
			// console.log(arr)
			var params2;
			$(".articleList-ul").html($(".articleList-ul li:first"));
			var fr_list = $(".articleList-ul").html();
			var rightList = '';
			for(var i=0; i<arr.length; i++) {
				var obj = arr[i];
				fr_list += '<li data-id1="'+ params.id +'" data-id2="'+ obj.id +'">';

                 if(obj.id==id2){
					 fr_list += '<a class=\"picactive\" href=\"javascript:;\">';
				 }
				 else{
					 fr_list += '<a  href=\"javascript:;\">';
				 }
				fr_list += '<div class="icondes">'+ obj.name +'</div>';
				fr_list += '</a>';
				fr_list += '</li>';
				$(".articleList-ul").html(fr_list);
			}

			init2(id1, id2);

			// 侧边切换
			$(document).on("click", ".articleList-ul > li > a", function(){
				var id1 = $(this).parent().data("id1");
				var id2 = $(this).parent().data("id2");
				$("li[data-id2="+ id2 +"]").find("a").addClass("picactive").parent().siblings().find("a").removeClass("picactive");
				$(".titleContent").html("");
				init2(id1, id2);
			});

		},
		error: function() {
			alert("服务繁忙，请稍后再试！");
		}
	});
};

// 三级菜单
function init2(id1, id2) {
	var params2 = {
		cityCode: cityCode,
		hospitalId:hospitalId,
		id1: id1,
		id2: id2,
		token: token
	};
console.log(params2)
	$.ajax({
		type: 'GET',
		url: config.appserver_url + '/serviceItem/queryServiceItem.json',
		data: params2,
		dataType: 'json',
		success: function(response) {
			console.log(response)
			tokenLose(response.status)
			var list3 = '';
			var arr3 = response.data;
			for(var k=0; k<arr3.length; k++) {
				var obj1 = arr3[k];
				var arrHeads = obj1.head.split('-');
				list3 += '<div class="fr_ecovery">'
				list3 += '<div class="fr_ecoverylist">';
				list3 += '<ul class="clearfix">'
				var obj2 = obj1.body;
				for(var j=0; j<obj2.length; j++) {
					var oBody = obj2[j]
					list3 += '<li data-serviceid="'+ oBody.serviceId +'">';
					list3 += '<a href="javascript:;">';
					list3 += '<div class="r_list_head">';
					list3 += '<span><img src="'+ oBody.picUrl +'" alt=""></span>';
					list3 += '</div>';
					list3 += '<div class="r_list_des">';
					list3 += '<p class="defaulticon">'+ oBody.serviceName +'</p>';
					list3 += '<div class="resprice clearfix">'
					list3 += '<p class="reserveper"><span class="drug-number">112</span>丸*<span class="bottle-number">3</span>瓶';
					list3 += '<p class="price">';
					list3 += '<span class="now"><span class="nowprice"><span class="symbol">￥</span>'+ oBody.price +'</span><span class="yuanTime">元/次</span></span><span class="old"></span>';
					list3 += '</p>';
					list3 += '</div>';
					list3 += '</div>';
					list3 += '</a>';
					list3 += '</li>';
				}
				list3 += '</ul>';
				list3 += '</div>';
				list3 += '</div>';
				$(".titleContent").html(list3);

				$(document).on("click", ".fr_ecoverylist ul > li", function() {
					var serviceId = $(this).data("serviceid");
					var nextPage ="../myClinic/drugIntroduction.html?id1="+ id1 + "&hospitalId="+ hospitalId  +"&serviceId="+ serviceId;
					historyForward(nextPage);
				});
			}

		}
	})
};

// 获取本地数据
function getItems() {
	token = JSON.parse(localStorage.getItem("user")).token;
	cityCode = JSON.parse(sessionStorage.getItem("localCityInfo")).localCityCode;
	cityCode = sessionStorage.getItem("cityCode");
	hospitalId =GetQueryStr("hospitalId");
};

$(document).on("click", "#share", function() {
	if($(".absoWrap").css("left") == "0px") {
		$(".absoWrap").css("left", "-100%");
	}else{
		$(".absoWrap").css("left", "0");
	};
});
