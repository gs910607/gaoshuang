...
if(!JSON.parse(localStorage.getItem("user"))) {
	sessionStorage.setItem("noLogin", location.href.split("PP2.0/views")[1]); //登录时接收，登陆后跳回此页面
	alert("没有用户信息 请重新登录!")
	setTimeout(function() {
...
function queryLast() {
	...
	$.ajax({
		type: "get",
		data: params,
		url: config.appserver_url + "/myClinic/queryMyDefaultClinic.json",
		dataType: "json",
		async: true,
		success: function(data) {
			console.log(data)
			...
			console.log(data.data)
			hospitalId = data.data.hospitalVo.id;
      ...
      $(".promptContent>h4").text("拨号");
			$(".promptContent>p").text("您确认要拨打" + data.data.hospitalVo.contactsPhone + "吗？")
			//轮播图首图片更换
...
function getLocalCity() {
	...
	geolocation.getCurrentPosition(function(r) {
		if(this.getStatus() == BMAP_STATUS_SUCCESS) {
			...
			if(localCityCode && localCityName) {
