...
if (articleId) {
	$.ajax({
		type : 'POST',
		url : '../../politics/getPoliticsById.do',
		data : {
			id : articleId
		},
		dataType : 'json',
		success : function(response) {
			...
			$("#messageDigest").val(response.type);
			$("#msgHeader").val(response.title);
			$("#messageContent").val(response.content);
			$("#politicsId").val(response.id);
			selectChange();
		},
		error : function() {
			ajaxLoading.hide();
...
function submitPicture() {
	...
	var cPoliticsType = $("#messageDigest").val(); //类别
	var cPoliticsTitle = $("#msgHeader").val(); //标题
	var cPoliticsContent = $("#messageContent").val(); //内容
	var id = $("#LegalId").val();
	if (id != null && id != "") {
		formData.append("cPoliticsId", id);
	}
	if ($("#upPicture")[0].files[0] != undefined) {
		formData.append("file", $("#upPicture")[0].files[0]); //图片
	}
	formData.append("cPoliticsType", cPoliticsType);
	formData.append("cPoliticsTitle", cPoliticsTitle);
	formData.append("cPoliticsContent", cPoliticsContent);

	$.ajax({
		url : '../../politics/addPolitics.do',
		type : 'POST',
		data : formData,
		// 告诉jQuery不要去处理发送的数据
		processData : false,
		...
		success : function(responseStr) {
			if (responseStr.status == 1) {
				...
				location.href = "politicsHotList.html"
			} else {
