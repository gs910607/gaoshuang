...
$(function() {
	...
	$.post("/xlhaBeta/legal/getPageList.do",{sizePage: config.sizePage}, function(data) {
		...
		Page({
			...
			callback : function(n) { //回调函数
				$.post("/xlhaBeta/legal/getPageList.do?pages=" + n,{sizePage: config.sizePage}, function(data) {
...
function deleteLegal(data) {
	...
	if(confirm("是否删除?")) {
		$.post("/xlhaBeta/legal/deleteLegal.do", {
			id : data
		}, function(respose) {
			if (respose.status == 1) {
				alert(respose.msg);
...
function getSearch(n){
	...
	$.ajax({
		url : '/xlhaBeta/legal/getPageList.do?',
		type : 'POST',
		data : params,
		dataType: 'json',
		beforeSend : function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			Page({
				...
				elem : $('#page'), //指定的元素
				callback : function(n) { //回调函数
					$.post("/xlhaBeta/legal/getPageList.do?",{cLegalTitle: cLegalTitle,pages:n,sizePage: config.sizePage}, function(data) {
