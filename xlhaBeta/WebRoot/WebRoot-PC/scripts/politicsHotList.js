...
$(function() {
	...
	$.post("/xlhaBeta/politics/getPageList.do",{sizePage: config.sizePage}, function(data) {
		...
		Page({
			...
			callback : function(n) { //回调函数
				$.post("/xlhaBeta/politics/getPageList.do?pages=" + n, {sizePage: config.sizePage}, function(data) {
					setPolitics(data);
				});
			}
		});
...
function deleteLegal(data) {
	isDelete("是否删除？", function() {
		$.post("/xlhaBeta/politics/deletePolitics.do", {
			id : data
		}, function(respose) {
			if (respose.status == 1) {
				alert(respose.msg);
...
function getSearch(n){
	...
	$.ajax({
		url : '/xlhaBeta/politics/getPageList.do?pages='+n,
		type : 'POST',
		data : params,
		beforeSend : function() {
			ajaxLoading.show();
...
$("#html5Form").html5Validate(function() {
	...
	$.ajax({
		url : '/xlhaBeta/politics/getPageList.do',
		type : 'POST',
		data : params,
		beforeSend : function() {
			ajaxLoading.show();
